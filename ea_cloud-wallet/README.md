...
├── app/                ← Google-Maps shell (com.diegonmarcos.cloudwallet)
└── libs/
...
Dev-Control HTTP), rendering **Cloud Wallet's own** data. The four cross-subsystem
...
./build.sh build      # produces dist/Cloud-Wallet.apk
...
`com.diegonmarcos.superapp.*` → `com.diegonmarcos.cloudwallet.*`.
