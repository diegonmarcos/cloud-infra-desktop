#!/usr/bin/env bash
# redroid-apps — tester. Pure tests run anywhere; live tests need the Redroid container up.
set -euo pipefail
...
if node -e "
  ...
  const pkgs=new Set(c.apps.map(a=>a.package));
  // folders: skip '_'-prefixed doc keys (data-driven _doc convention), like the engine does.
  const fids=new Set(Object.keys(l.folders).filter(k=>!k.startsWith('_')));
  ...
  for(const [k,f] of Object.entries(l.folders)){ if(k.startsWith('_')||!f||!Array.isArray(f.members)) continue;
    for(const m of f.members) if(!pkgs.has(m)) throw 'folder member not an app: '+m; }
  l.hotseat.forEach(chkRef); l.workspace.cells.forEach(c=>chkRef(c.ref));
"; then ok "refs resolve"; else no "refs resolve"; fi

echo "== T3: workspace cells fit the grid and don't collide (per screen) =="
if node -e "
  ...
  for(const c of l.workspace.cells){
    ...
    const k=(c.screen||0)+':'+c.cellY+','+c.cellX; if(seen.has(k)) throw 'cell collision at '+k; seen.add(k);
  }
...
else echo "  (skip — no dist/apks yet; run ./build.sh build)"; fi

echo "== T7 (live): hotseat + workspace populated =="
cname="$(node -e "process.stdout.write(require('$CONFIG').redroid.container_name)")"
if docker ps --format '{{.Names}}' 2>/dev/null | grep -qx "$cname"; then
  root="$(node -e "process.stdout.write(require('$CONFIG').redroid.data_volume)")"
  glob="$(node -e "process.stdout.write(require('$CONFIG').redroid.launcher_db_glob)")"
  db="$(sudo find "$root" -path "$root/$glob" 2>/dev/null | head -1)"
  if [ -n "$db" ]; then
...
else echo "  (skip — container '$cname' not running)"; fi
