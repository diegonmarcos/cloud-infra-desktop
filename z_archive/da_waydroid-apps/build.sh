...
cmd_lock() {
  ...
  # Optional package filter: `build.sh lock [pkg ...]` relocks ONLY those
  # packages, MERGING into the existing lockfile (surgical pin refresh — e.g.
  # when F-Droid prunes an old APK so its pinned URL 404s). No args = full
  # regen from scratch (all apps), which is all-or-nothing by design.
  local -a filter=( "$@" )
