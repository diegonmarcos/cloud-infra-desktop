 ...
 *
 * Verbatim port of Cloud-SuperApp's SysfsProc (the About page's
 * SYSFS-PROC section reader) into the Cloud-Comms hub — only the
 * package changed; the hub must not import com.diegonmarcos.superapp.*.
 *
 ...
 *                         report mA; same |x| < 100_000 heuristic
 *                         auto-rescales)
...
object SysfsProc {
     ...
     *  count; divide by `clkTck()` for seconds. Returns empty when
