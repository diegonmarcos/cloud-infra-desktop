...
> 5. **One download (embedded-installer)**: the forks are still separate APKs (separate DBs / clean upstream sync) but ship bundled INSIDE the single Cloud-Comms APK — `build.sh bundle-forks` embeds each published fork into `hub/src/main/assets/forks/<domain>.apk`; on first launch the hub installs them via PackageInstaller (`BundledForkInstaller`, one confirm per fork). NOT a true single-merged APK (rejected — RN+Rust+Java merge war). FleetUpdater keeps them current from GHCR after.
