...
12. Copies the entire install dir → `/home/diego/git/unix/rescue-os-debian/` so the user can re-run `./install.sh` without unlocking the pool first.
...
```bash
sudo ~/git/unix/rescue-os-debian/bootstrap/install-debian.sh
```
