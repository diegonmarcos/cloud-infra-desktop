#!/usr/bin/env bash
# nixos-cp-window — left-click control panel: section sidebar (left tabs) + item list
...
# nixos-systray in same bin/ handles action dispatch via --run-index N
...
for (( s=0; s < section_count; s++ )); do
  ...
  # Global flat index for section $s = sum of item-counts in sections[0..s-1] + local item index
  LIST_ARGS=()
