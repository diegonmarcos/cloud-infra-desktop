...
if [ ! -x "$REAL" ]; then
  ...
  say "run: cd ~/git/cloud-infra-desktop/bb_flakes_termux && ./build.sh switch   then re-run this script"
...
if [ ! -x "$REAL" ]; then
  say "FAIL: $REAL missing or not executable — run: cd ~/git/cloud-infra-desktop/bb_flakes_termux && ./build.sh switch"
  fail=1
else
