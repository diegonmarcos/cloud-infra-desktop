...
# Get the installed claude-code version on termux (npm global)
get_termux_claude_version() {
  local global_bin="/data/data/com.termux.nix/files/usr/bin/claude"
  if [[ -x "$global_bin" ]]; then
    "$global_bin" --version 2>/dev/null | head -1 | grep -oP '[\d.]+' | head -1 || true
  fi
}
...
collect_custom_derivations() {
  ...
  # claude-code (termux) — npm global
  if [[ "$PLATFORM" == "termux" ]]; then
    local cc_termux_current cc_termux_latest
    cc_termux_current=$(get_termux_claude_version)
    cc_termux_latest=$(get_latest_npm "@anthropic-ai/claude-code")
    add_result "claude-code" "termux" "${cc_termux_current:-?}" "${cc_termux_latest:-?}" "npm-global" "npm global" "" "n/a"
  fi >> "$tmpdir/results"
