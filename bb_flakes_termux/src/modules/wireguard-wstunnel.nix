...
=======
  ...
  home.file.".local/bin/wg-tcp" = {
    ...
    text = builtins.replaceStrings
      [ "@BASH@"       "@PID_FILE@" "@LOG_FILE@" "@SECRET_FILE@" "@WSTUNNEL_BIN@" "@LOCAL_UDP@"           "@REMOTE_WG@"          "@WS_ENDPOINT@" ]
      [ "${pkgs.bash}" pidFile      logFile      secretFile       wstunnelBin      (toString localUdp)     (toString remoteWg)    wsEndpoint      ]
      (builtins.readFile ./scripts/wg-tcp.sh);
  };
>>>>>>> Stashed changes
}
