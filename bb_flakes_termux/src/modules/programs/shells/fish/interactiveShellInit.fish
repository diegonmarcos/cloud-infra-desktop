...
if test -f ~/git/vault/A0_keys/providers/anthropic/api-key_opaque
  set -gx ANTHROPIC_API_KEY (command cat ~/git/vault/A0_keys/providers/anthropic/api-key_opaque)
end

# Vault env keys — fish dialect (generated by vault/build.sh env)
# Source of truth: ~/git/vault/vault-keys.json
if test -f ~/git/vault/vault-keys-fish.fish
  source ~/git/vault/vault-keys-fish.fish
end
