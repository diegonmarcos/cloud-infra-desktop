# ═══════════════════════════════════════════════════════════════════════════
# VENDORED NIXPKGS MODULE — nixos/modules/tasks/filesystems/btrfs.nix
# ═══════════════════════════════════════════════════════════════════════════
# WHY (2026-06-11): nixpkgs 24.11 hardcodes initrd module name
# "blake2b_generic", but mainline kernel >= 6.19 renamed the object to
# blake2b.ko (crypto/Makefile: CONFIG_CRYPTO_BLAKE2B += blake2b.o).
# makeModulesClosure runs with allowMissing=false, so building the initrd
# for linux-surface 6.19.8 fails:
#   modprobe: FATAL: Module blake2b_generic not found
# (caught by ship-boot-cache CI run 27351190964).
#
# FIX: nixpkgs master replaced the unstable file names with crypto
# ALGORITHM ALIASES ("xxhash64", "blake2b-256", "sha256") which every
# kernel registers via MODULE_ALIAS_CRYPTO — they resolve on 6.15 (to
# *_generic.ko) and on 6.19 (to the renamed modules) alike. This file is
# the pinned 24.11 module with only that list changed.
#
# REMOVE when the flake's nixpkgs input includes the upstream alias fix
# (check: rg blake2b <nixpkgs>/nixos/modules/tasks/filesystems/btrfs.nix
# shows "blake2b-256"). Then delete this file plus the disabledModules +
# imports lines in hardware_filesystems.nix.
# Tester: ship-boot-cache CI job "Build + push Nix-format kernel closure"
# must pass its initrd build step.
# ═══════════════════════════════════════════════════════════════════════════
{
  config,
  lib,
    ...
    (mkIf inInitrd {
      ...
      boot.initrd.availableKernelModules =
        ...
        ++ optionals (config.boot.kernelPackages.kernel.kernelAtLeast "5.5") [
          ...
          # VENDORED FIX: crypto algorithm ALIASES (stable across kernel
          # versions), not module file names (renamed in kernel 6.19).
          "xxhash64"
          "blake2b-256"
          "sha256"
        ];
