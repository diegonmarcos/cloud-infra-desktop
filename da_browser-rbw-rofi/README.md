...
Hotkey-driven password and TOTP autofill on Linux, backed by [`rbw`](https://github.com/doy/rbw)
(Bitwarden / Vaultwarden CLI). Replaces the Bitwarden browser extension for
the password+TOTP flow: pick an entry from a `rofi`/`wofi` picker, then have
the username and password typed straight into the focused field via `wtype`
(Wayland) or `xdotool` (X11). Single-shot, ~10 MiB peak RSS, no resident
daemon — the binary is invoked on a hotkey by `systemd --user`.
...
```sh
./build.sh enable     # systemctl --user enable --now da_browser-rbw-rofi.service
```
...
```sh
systemctl --user start da_browser-rbw-rofi.service
```
...
- Trigger: `Super+P`
- Action: `systemctl --user start da_browser-rbw-rofi.service`

### Hyprland

```hyprlang
bind = SUPER, P, exec, systemctl --user start da_browser-rbw-rofi.service
```

### Sway / i3

```text
bindsym $mod+p exec --no-startup-id systemctl --user start da_browser-rbw-rofi.service
```
...
gsettings set org.gnome.settings-daemon.plugins.media-keys custom-keybindings "['/org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/custom0/']" \
  ...
  && gsettings set org.gnome.settings-daemon.plugins.media-keys.custom-keybinding:/org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/custom0/ command 'systemctl --user start da_browser-rbw-rofi.service' \
...
typing speed. Set `DA_BROWSER_RBW_ROFI_SLOW=1` and re-trigger the hotkey:

```sh
DA_BROWSER_RBW_ROFI_SLOW=1 systemctl --user start da_browser-rbw-rofi.service
```
...
- **Nothing happens** — check the unit's status:
  `systemctl --user status da_browser-rbw-rofi.service`
...
- **Nothing types** — confirm the right typer is installed for your
  ...
  da_browser-rbw-rofi.service`) and adding
  `Environment=RUST_LOG=debug`. Check the journal:
  `journalctl --user -u da_browser-rbw-rofi.service -n 100`.
- **`build.json` not found** — by default the binary walks up from the
  current directory and from its own location. Override with
  `DA_BROWSER_RBW_ROFI_CONFIG=/abs/path/to/build.json`, or drop a copy
  under `$XDG_CONFIG_HOME/da_browser-rbw-rofi/build.json`.
...
|   |-- systemd/
|   |   `-- da_browser-rbw-rofi.service   user-level oneshot unit
|   `-- src/
...
`-- dist/
    `-- da_browser-rbw-rofi    release binary (after ./build.sh build)
```
