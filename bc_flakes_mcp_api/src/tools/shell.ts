// Shell tools — run arbitrary commands, npm, pip, etc.
...
export function registerShellTools(server: McpServer) {
  server.tool(
    "shell_exec",
    "Execute a bash command and return output (30s timeout, use with care)",
    {
    ...
    async ({ command, timeout, cwd }) => {
      ...
      const result = bash(command, { timeout: ms, cwd });
      return {
  ...
  server.tool(
    ...
    {},
    async () => {
      const result = bash(`npm list -g --depth=0 2>/dev/null`);
  ...
  server.tool(
    ...
    async ({ package: pkg }) => {
      const result = bash(`npm install -g "${pkg}" 2>&1`, { timeout: 120_000 });
      return {
