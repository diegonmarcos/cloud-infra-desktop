...
# ── ABI variant helpers ────────────────────────────────────────────────
# CLOUDCALENDAR_VARIANT (env) selects a release.variants[] entry. Unset = arm64
# default → every helper falls back to the legacy single-variant keys.
_variant_field() {
  local v="${CLOUDCALENDAR_VARIANT:-}"
  [ -z "$v" ] && return 0
...
# Export CLOUDCALENDAR_ABIS (CSV) for gradle from the active variant. No-op when
# unset → gradle reads build.json::android.abi_filters.
_export_variant_abis() {
  local csv; csv="$(_variant_field '.abis | join(",")')"
  if [ -n "$csv" ]; then
    export CLOUDCALENDAR_ABIS="$csv"
    log "Variant ${CLOUDCALENDAR_VARIANT:-}: ABIs=$csv"
  fi
  return 0
}
