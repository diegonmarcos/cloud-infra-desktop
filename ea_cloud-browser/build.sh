#!/usr/bin/env bash
# ╔══════════════════════════════════════════════════════════════════╗
# ║ Cloud Browser — Universal Build Dispatcher                            ║
# ║                                                                  ║
...
CMD="${1:-help}"
APP_MAIN="com.diegonmarcos.cloudbrowser.MainActivity"
...
# ── ABI variant helpers ────────────────────────────────────────────────
# CLOUDBROWSER_VARIANT (env) selects a release.variants[] entry. Unset = arm64
# default → every helper falls back to the legacy single-variant keys.
_variant_field() {
  local v="${CLOUDBROWSER_VARIANT:-}"
  [ -z "$v" ] && return 0
...
# Export CLOUDBROWSER_ABIS (CSV) for gradle from the active variant. No-op when
# unset → gradle reads build.json::android.abi_filters.
_export_variant_abis() {
  local csv; csv="$(_variant_field '.abis | join(",")')"
  if [ -n "$csv" ]; then
    export CLOUDBROWSER_ABIS="$csv"
    log "Variant ${CLOUDBROWSER_VARIANT:-}: ABIs=$csv"
  fi
  return 0
}
...
step_oras_pull() {
  ...
  asset_title="$(jq -r '.layers[0].annotations["org.opencontainers.image.title"] // "cloud-browser.apk"' <<<"$manifest")"
