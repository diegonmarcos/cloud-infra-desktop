#!/usr/bin/env bash
# Tester: zoomies pets extracted from app/ into a proper libs/zoomies module.
#
...
# extraction to ea_cloud-superapp/libs/zoomies is wired the same way as the other
...
echo "== T1: libs/zoomies module files exist =="
...
python3 - "$ROOT/build.json" <<'PY' && ok "build.json declares libs:zoomies + app depends on it" || bad "build.json wiring missing"
import json,sys
d=json.load(open(sys.argv[1]))
m=d.get("modules",{})
assert "libs:zoomies" in m, "no libs:zoomies module entry"
assert "libs:zoomies" in m["app"]["depends_on"], "app does not depend on libs:zoomies"
PY
has "app/build.gradle" "project(':libs:zoomies')" "app/build.gradle implements :libs:zoomies"
