...
for pair in $FORKS; do
  key="${pair%%:*}"; wf="$UNIX/1_workflows/src/cicd/ship-cloud-$key.yml"
  ...
  grep -qF 'build.sh gh-release-fork' "$UNIX/.github/workflows/ship-cloud-$key.yml" 2>/dev/null \
    && ok ".github ship-cloud-$key.yml in sync" || bad ".github ship-cloud-$key.yml NOT redeployed (run 1_workflows/build.sh)"
done
