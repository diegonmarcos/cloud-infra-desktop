...
(async function () {
  ...
  window.addEventListener("resize", () => { if (Tabs.active) MYK._fitTab(Tabs.active); NativeBrowser.sync(); });
  ...
  const applyLayout = () => {
    ...
    NativeBrowser.sync();   // same gap as resize — a browser tab's host just moved/resized too
  };
