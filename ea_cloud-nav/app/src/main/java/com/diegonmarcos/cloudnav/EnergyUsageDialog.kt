 ...
 *      AccuBattery's per-app list).
 *   4. "Inside Cloud Nav" — our own subsystem ledger
...
class EnergyUsageDialog : DialogFragment() {
    ...
    private fun build(ctx: Context, root: LinearLayout) {
        ...
        //    screen-on discharge mA). No privilege; just Usage Access.
        //    Same method AccuBattery uses for its per-app list.
        card(ctx, root, "By app  ·  estimate (foreground × draw)") { box ->
            if (!EnergyWatchdog.hasUsageAccess(ctx)) {
                box.addView(small(ctx, "Needs Usage Access. Grant it in Configs/About → Permissions → Set Usage Access."))
                return@card
            }
        ...
        card(ctx, root, "Inside Cloud Nav") { box ->
            if (ledger.isEmpty()) {
