...
import com.diegonmarcos.cloudnav.AppProcessUptime
import com.diegonmarcos.cloudnav.BuildConfig
import com.diegonmarcos.cloudnav.PermAskTracker
import com.diegonmarcos.cloudnav.SysfsProc
import com.diegonmarcos.cloudnav.ProfilePrefs
import java.io.File
 ...
 *
 * Ported verbatim from Cloud SuperApp's DevControlFragment, MINUS the
 * cross-subsystem sections that Cloud Nav doesn't ship (VPN/WireGuard/
 * Mesh, Health Connect, Floating Nav, Shizuku exact-energy) and the
 * launcher-only sections (Release/GHCR, Sections-count, Lock-screen).
 */
class DevControlFragment : Fragment() {
     ...
     *  user grants SYSTEM_ALERT_WINDOW. Falls back to the global list, then
     *  this app's details screen. */
     ...
     *  after the bulk-grant. Pre-API-30 falls back to the
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, s: Bundle?): View {
        ...
        section(ctx, column, "Permissions") {
            ...
            //    The holder is the Cloud-Comms phone fork, NOT this app;
            ...
            it.addView(permButtonRow(ctx,
                permButton(ctx, "Set Usage Access", com.diegonmarcos.cloudnav.EnergyWatchdog.hasUsageAccess(ctxAny())) { openUsageAccessSettings() },
            ...
            //    The app can't grant another app a role, so jump to the
        ...
        section(ctx, column, "Battery & Usage") {
            ...
            val bs = com.diegonmarcos.cloudnav.BatterySessionStats.read(ctxAny)
            ...
            row(ctx, it, labelSince,
                com.diegonmarcos.cloudnav.BatterySessionStats.fmtSinceAnchor(bs))
            row(ctx, it, labelRate,
                com.diegonmarcos.cloudnav.BatterySessionStats.fmtRateUnified(bs))
            row(ctx, it, labelPower,
                com.diegonmarcos.cloudnav.BatterySessionStats.fmtPowerRow(bs))
            if (bs.isCharging) {
                row(ctx, it, "Charger input",
                    com.diegonmarcos.cloudnav.BatterySessionStats.fmtChargerSpec(bs))
                val phone = com.diegonmarcos.cloudnav.BatterySessionStats.fmtPhoneConsumption(bs)
                if (phone.isNotEmpty()) {
                    row(ctx, it, "Phone consumption", "$phone  (charger − battery)")
                }
            }
            row(ctx, it, labelEta,
                com.diegonmarcos.cloudnav.BatterySessionStats.fmtEtaDuration(bs))
            row(ctx, it, labelWall,
                com.diegonmarcos.cloudnav.BatterySessionStats.fmtEtaWallClock(bs))
            ...
            row(ctx, it, "Battery temp",
                com.diegonmarcos.cloudnav.BatterySessionStats.fmtBatteryTemp(bs))
            row(ctx, it, "Cycle count",
                com.diegonmarcos.cloudnav.BatterySessionStats.fmtCycleCount(bs))
            ...
            it.addView(actionButton(ctx, "⚡ Open Energy Usage (AccuBattery-style)") {
                runCatching {
                    com.diegonmarcos.cloudnav.EnergyUsageDialog()
                        .show(parentFragmentManager, com.diegonmarcos.cloudnav.EnergyUsageDialog.TAG)
                }
            })
        }
        ...
        // IPC Contract — the cross-app intent/IPC surface Cloud Nav
        // exposes/consumes. Always rendered, even when nothing is
        ...
        section(ctx, column, "Curl shortcuts") {
            ...
            // Endpoint catalog uses the /api/{group}/{op} layout; the
            // flat aliases still resolve but the documented form is the
            // grouped one. Source of truth for the full list is
            // GET /api/docs (machine-readable JSON, generated from the
            // same Spec list the routing uses so it can't drift). These
            // shortcuts cover the most-used probes.
        ...
        section(ctx, column, "Stack") {
            ...
            // to "completed" of the GHA `ship-cloud-nav.yml`
    ...
    private fun row(ctx: Context, host: LinearLayout, key: String, value: String): TextView {
        ...
        // later can rebind .text without rebuilding the row. Plain
     ...
     *  states and live toggles can re-skin the same view after creation.
     *  Palette convention:
    ...
    private fun buildAllPermsStatus(ctx: Context): String = buildString {
        appendLine("Cloud Nav — permission status")
    ...
    /** Live holder state for a single-holder RoleManager role. The app
