...

{
  imports = [
    ...
    # KWin window rules (data-driven, ./window-rules.json). Currently pins the
    # three build.sh/switch progress windows to Desk3 so a rebuild never buries
    # the desktop that launched it. Replaces the mechanism plasma.nix's stale
    # comment below attributes to session-template.nix — that file no longer
    # exists, so there were no window rules at all.
    ./window-rules.nix
  ...
  programs.plasma = {
    ...
    # window-rules live in ./window-rules.nix, data-driven from
    # ./window-rules.json (imported at the top of this file).
    #
    # The previous comment here credited ./session-template.nix +
    # ./session-template.json — NEITHER FILE EXISTS (removed at some point
    # without updating this note), so between then and 2026-08-12 there was no
    # window-rule mechanism at all and the claim sent anyone looking for one to
    # a dead end. Adding a pinned window means editing window-rules.json.
  ...
  # ─────────────────────────────────────────────────────────────────
  # Custom plasmoid — com.diegonmarcos.watchdog (my-konsole panel widget)
  # ─────────────────────────────────────────────────────────────────
  ...
  #
  # The widget renders data published by my-konsole's tray daemon but shares
  # no code with the app, so nothing is double-managed by moving it: the app
  # still owns its binary, launcher and .desktop entry via build.json.
  home.activation.installWatchdogPlasmoid = lib.hm.dag.entryAfter [ "writeBoundary" "configure-plasma" ] ''
    DEST="$HOME/.local/share/plasma/plasmoids/com.diegonmarcos.watchdog"
    rm -rf "$DEST"
    mkdir -p "$(dirname "$DEST")"
    cp -r --no-preserve=mode,ownership "${../dotfiles/kde/plasmoids/com.diegonmarcos.watchdog}" "$DEST"
    chmod -R u+w "$DEST"
  '';
