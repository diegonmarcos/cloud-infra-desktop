  ...
  home.packages = with pkgs; [
    ...
    (pkgs.writeShellScriptBin "claude"
      (builtins.replaceStrings
        [ "@glibcLib@" ]
        [ "${pkgs.glibc}/lib" ]
        (builtins.readFile ./scripts/claude.sh)))
...
=======
    ...
    (pkgs.writeShellScriptBin "claude-termux" (builtins.readFile ./scripts/claude-termux.sh))

    # NOTE: also provided by the my-ai binary self-extract (~/.local/bin); redundant, removal deferred.
    (pkgs.writeShellScriptBin "claude-malloc" (builtins.readFile ./scripts/claude-malloc.sh))

    # NOTE: also provided by the my-ai binary self-extract (~/.local/bin); redundant, removal deferred.
    (pkgs.writeShellScriptBin "claude-rescue" (builtins.readFile ./scripts/claude-rescue.sh))
>>>>>>> Stashed changes
  ];
}
