...
- **Source**: `~/git/cloud-infra-desktop/ba_flakes_desktop/src/`
- **Engine**: `~/git/cloud-infra-desktop/ba_flakes_desktop/build.sh`
...
| 2c  | ✓ verified | bb_flakes_termux build (HM termux): success — proot-static path error is pre-existing cross-arch artefact (this host x86_64, termux aarch64-android), unrelated to refactor. aa_desk-usr_x86_surface-linux_nixos check (NixOS): 47.7s dry-run, clean. | — |
...
| Final | ✓ verified | All three flakes build clean: ba_flakes_desktop (`fn4v5cqp...`, 17.3s), bb_flakes_termux (build OK, pre-existing cross-arch proot-static noise unchanged), aa_desk-usr_x86_surface-linux_nixos (22.6s dry-run, "Git tree dirty" warning is expected from uncommitted work). | — |
...
Common test runner: `~/git/cloud-infra-desktop/ba_flakes_desktop/build.sh build` — engine is the
