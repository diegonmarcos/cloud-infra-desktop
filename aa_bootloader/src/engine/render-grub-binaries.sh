...
#                                    → dist/boot/efi$EFI_INSTALL_DIR/grubx64.efi
# EFI_INSTALL_DIR is read from boot.json (.grub.install.efi_install_dir);
# default is /EFI/bootloader (neutral name — no distro owns the binary,
# aa_bootloader is the SoT and overwrites it on every deploy).
#
...
if command -v grub-install >/dev/null 2>&1; then
    ...
    # Modules dir candidates: standard distros first (Debian/Ubuntu/Arch/Fedora),
    # then NixOS layout (/run/current-system/sw/lib/grub/x86_64-efi).
    for cand in /usr/lib/grub/x86_64-efi /lib/grub/x86_64-efi /usr/lib/grub2/x86_64-efi \
                /run/current-system/sw/lib/grub/x86_64-efi; do
        [ -d "$cand" ] && { GRUB_MODULES_DIR="$cand"; break; }
    done
    for cand in /usr/share/grub/unicode.pf2 /usr/share/grub2/themes/unicode.pf2 \
                /usr/share/grub2/unicode.pf2 /run/current-system/sw/share/grub/unicode.pf2; do
...
if [ -n "${GRUB_MKIMAGE:-}" ] && [ -x "$GRUB_MKIMAGE" ]; then
    ...
    # ── Embedded pre-config (FIRE RULE 4: data-driven from boot.json) ─────
    # grubx64.efi loads from the FAT ESP, but our grub.cfg + modules live on
    # the ext4 /boot partition (different UUID). Without an embedded config,
    # GRUB tries to find grub.cfg at <prefix>/grub.cfg on the ESP and fails
    # → drops to the rescue shell.
    #
    # The embedded.cfg below runs BEFORE GRUB looks for its main grub.cfg.
    # It uses search.fs_uuid to locate the ext4 /boot partition by the UUID
    # declared in boot.json (.uefi.boot.uuid), sets $root, then sets $prefix
    # so subsequent module + grub.cfg loads land on /boot/grub/.
    EMBEDDED_CFG="$DIST_GRUB/embedded.cfg"
    cat > "$EMBEDDED_CFG" <<EOF
# AUTO-GENERATED by render-grub-binaries.sh — baked into grubx64.efi.
# Locates the ext4 /boot partition (UUID=$BOOT_UUID, declared in boot.json),
# then sets prefix so grubx64.efi can find its modules + main grub.cfg.
search.fs_uuid $BOOT_UUID root
set prefix=(\$root)/grub
EOF

    # -p (memdisk): tells grub-mkimage to bake the embedded config into a
    # memdisk inside the binary; $prefix is set to (memdisk) on first boot.
    # -c <embedded.cfg>: the config that runs BEFORE prefix-resolution to
    # discover the real /boot location and override $prefix.
    ...
    log "  embedded prefix-search for boot UUID=$BOOT_UUID"
else
