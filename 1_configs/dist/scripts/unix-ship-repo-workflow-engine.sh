...
# ║                                                                  ║
# ║   Source : 1_configs/src/gha/scripts/unix-ship-repo-workflow-engine.sh
...
export REPO_ROOT
export ENGINE_NAME="1_configs/src/gha/scripts/unix-ship-repo-workflow-engine.sh"
...
do_build() {
    ...
    if ! ls "$SRC_DIR"/gha/cicd/*.yml "$SRC_DIR"/gha/cicd/*.yaml >/dev/null 2>&1; then
        echo "FATAL: no workflows found at $SRC_DIR/gha/cicd/*.{yml,yaml}" >&2
        exit 1
    fi
    for f in "$SRC_DIR"/gha/cicd/*.yml "$SRC_DIR"/gha/cicd/*.yaml; do
        [ -f "$f" ] || continue
